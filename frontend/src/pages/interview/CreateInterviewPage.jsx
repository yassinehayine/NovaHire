...
const INTERVIEW_STYLES = [
  { value: 'MIXED',         label: 'Mixed',         sub: 'All question types',       icon: '⚡' },
  { value: 'TECHNICAL',     label: 'Technical',     sub: 'Code, algorithms, debugging', icon: '🔧' },
  { value: 'BEHAVIORAL',    label: 'Behavioral',    sub: 'Soft skills & STAR method',  icon: '💬' },
  { value: 'SYSTEM_DESIGN', label: 'System Design', sub: 'Architecture & scalability', icon: '🏗️' },
]
...
const StepSettings = ({ form, onChange, errors }) => (
  <div className="space-y-8">
    ...
    <div>
      <p className="label flex items-center gap-2">
        <Cpu size={15} className="text-nova-400" />
        Interview Style
      </p>
      <div className="grid grid-cols-2 gap-3">
        {INTERVIEW_STYLES.map(({ value, label, sub, icon }) => (
          <button
            key={value}
            type="button"
            onClick={() => onChange({ target: { name: 'interviewStyle', value } })}
            className={`p-3 rounded-xl border text-left transition-all duration-150
              ${form.interviewStyle === value
                ? 'border-nova-500 bg-nova-600/20'
                : 'border-white/10 bg-white/5 hover:bg-white/8 hover:border-white/20'}`}
          >
            <p className="text-base mb-0.5">{icon}</p>
            <p className={`font-semibold text-sm ${form.interviewStyle === value ? 'text-nova-200' : 'text-slate-200'}`}>{label}</p>
            <p className="text-xs text-slate-500 mt-0.5">{sub}</p>
          </button>
        ))}
      </div>
...
const ReviewSummary = ({ form }) => {
  ...
  const style = INTERVIEW_STYLES.find(s => s.value === form.interviewStyle)
    ...
    <div className="space-y-4">
      ...
      <div className="bg-surface-2 rounded-xl border border-white/8 divide-y divide-white/5">
        {[
          ...
          { icon: Zap,     label: 'Style',        value: `${style?.icon ?? '⚡'} ${style?.label ?? 'Mixed'}` },
          { icon: Globe,   label: 'Language',     value: `${lang?.flag} ${lang?.label}` },
          { icon: Cpu,     label: 'Questions',    value: `~${questionCount} questions (AI-generated)` },
...
const CreateInterviewPage = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success('Interview created! AI is generating your questions…')
